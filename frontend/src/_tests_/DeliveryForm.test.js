...
describe("DeliveryForm Component", () => {
  ...
  test("submits the form with valid data", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/order id:/i), { target: { value: "DEL123" } });
    fireEvent.change(screen.getByLabelText(/status:/i), { target: { value: "Pending" } });
    fireEvent.change(screen.getByLabelText(/customer:/i), { target: { value: "John Doe" } });
    fireEvent.submit(screen.getByRole("button", { name: /submit/i }));
  ...
  test("displays error message if server fails", async () => {
    fetch.mockImplementationOnce(() =>
      Promise.resolve({
        ok: false,
        json: () => Promise.resolve({ message: "Failed to connect to the server." }),
      })
    );
    ...
    await waitFor(() => expect(alert).toHaveBeenCalledWith("Failed to connect to the server."));
  });
});
